...
from pokemon import retrieve_pokemons_by_type, retrieve_pokemons_by_type_from_file
...
def test_retrieve_pokemons_by_type_when_type_not_in(grass_type_pokemon, water_type_pokemon):
    # criamos um arquivo em memória que o seu conteúdo são os dois pokemons
    fake_file = StringIO(
        json.dumps({"results": [grass_type_pokemon, water_type_pokemon]})
    )
    ...
    with pytest.raises(AssertionError):
        assert water_type_pokemon in retrieve_pokemons_by_type("Grass", fake_file)
...
def test_retrieve_pokemons_by_type():
    ...
    with patch("builtins.open", mock_open(read_data=pokemon_json_content)):
        ...
        assert retrieve_pokemons_by_type_from_file("Grass", "dummy") == [
            grass_type_pokemon
        ]